...
def random_rotate_image(image, angle):
    return scipy.ndimage.rotate(image, angle, mode='nearest')
...
def random_mask(config, name='mask', dtype=tf.float32):
    with tf.compat.v1.variable_scope(name), tf.device('/cpu:0'):
        ...
        nd = tf.compat.v1.random_uniform([ ], minval=0, maxval=n_masks-1, dtype=tf.int32)
        path_mask = path_list[nd]
        contents = tf.compat.v1.read_file(path_mask)
        mask = tf.image.decode_jpeg(contents, channels=3)
        mask = tf.compat.v1.image.random_flip_left_right(mask)
        #print('shape', mask.get_shape().as_list())
        angle = tf.random.uniform([ ], minval= -90., maxval= 90., dtype=dtype)
        scale = tf.random.uniform([ ], minval=0.8, maxval=1.0, dtype=dtype)
        mask = tf.compat.v1.py_func(random_rotate_image, [mask, angle], tf.uint8)
        #print('shape', mask.get_shape().as_list())
